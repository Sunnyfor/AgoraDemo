...
public abstract class RtcBaseActivity extends BaseActivity implements EventHandler {
    ...
    private void configVideo() {
        VideoEncoderConfiguration configuration = new VideoEncoderConfiguration(
                Constants.INSTANCE.getVIDEO_DIMENSIONS()[config().getVideoDimenIndex()],
                VideoEncoderConfiguration.FRAME_RATE.FRAME_RATE_FPS_15,
                VideoEncoderConfiguration.STANDARD_BITRATE,
                VideoEncoderConfiguration.ORIENTATION_MODE.ORIENTATION_MODE_FIXED_PORTRAIT
        );
        configuration.mirrorMode = Constants.INSTANCE.getVIDEO_MIRROR_MODES()[config().getMirrorEncodeIndex()];
    ...
    private void joinChannel() {
        ...
        String token = getString(R.string.private_access_token);
        ...
        if (local) {
            rtcEngine().setupLocalVideo(
                    new VideoCanvas(
                            ...
                            0,
                            Constants.INSTANCE.getVIDEO_MIRROR_MODES()[config().getMirrorLocalIndex()]
                    )
            );
        } else {
            rtcEngine().setupRemoteVideo(
                    new VideoCanvas(
                            ...
                            VideoCanvas.RENDER_MODE_HIDDEN,
                            uid,
                            Constants.INSTANCE.getVIDEO_MIRROR_MODES()[config().getMirrorRemoteIndex()]
                    )
            );
        }
        return surface;
